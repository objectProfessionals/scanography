package com.op.spectogram;

import com.op.Base;

import javax.imageio.ImageIO;
import java.awt.*;
import java.awt.geom.Rectangle2D;
import java.awt.image.BufferedImage;
import java.io.File;
import java.io.IOException;
import java.util.*;
import java.util.List;

public class Spectroscopy extends Base {

    public static Spectroscopy wheel = new Spectroscopy();
    private String type = "ANH";
    private String file1 = type + "specOut";
    private String ipF = hostOpDir + type + "/" + type + "_LINE1.png";
    private String opF = hostOpDir + "ANH/" + file1 + ".png";
    private int w = 1000;
    private int h = w;
    private BufferedImage ibi;
    private BufferedImage obi;
    private Graphics2D opG;

    public static void main(String[] args) throws Exception {
        wheel.draw();
    }

    private void draw() throws IOException {
        initFile();

        int dd = 1;
        int count = 0;
        int sep = 1;
        HashMap<Color, Integer> col2num = new HashMap<>();
        TreeMap<Float, Color> v2col = new TreeMap<>();
        double maxNum = 0;
        for (int i = 0; i < ibi.getWidth(); i = i + dd) {
            int rgb = ibi.getRGB(i, 0);
            int r = (rgb >> 16) & 0x000000FF;
            int g = (rgb >> 8) & 0x000000FF;
            int b = (rgb) & 0x000000FF;

            int rr = sep * (r / sep);
            int gg = sep * (g / sep);
            int bb = sep * (b / sep);

            Color c = new Color(rr, gg, bb);

            float[] hsv = new float[3];
            Color.RGBtoHSB(c.getRed(), c.getGreen(), c.getBlue(), hsv);
            v2col.put(hsv[0]*hsv[1]*hsv[2], c);

            if (col2num.get(c) == null) {
                col2num.put(c, 1);
            } else {
                int newVal = col2num.get(c).intValue() + 1;
                col2num.put(c, newVal);
                if (maxNum < newVal) {
                    maxNum = newVal;
                }
            }
            count++;
        }

        System.out.println(maxNum);

        //drawRef();
        //drawPlots(hm, maxNum, true);
        drawPlots(v2col, col2num, maxNum, false);

        System.out.println("v2col=" + v2col.keySet().size());
        System.out.println("col2num=" + col2num.keySet().size());

        System.out.println("count=" + count);
        savePNGFile(obi, opF, 300);
    }

    private void drawRef() {
        int th = w / 200;
        int ii = w / 2;
        for (int i = Color.BLACK.getRGB(); i < Color.WHITE.getRGB(); i = i + ii) {
            Color col = new Color(i);
            double[] xyz = getRefPositions(th, col.getRed(), col.getGreen(), col.getBlue());
            int x = (int) xyz[0];
            int y = (int) xyz[1];
            float[] hsv = new float[3];
            Color.RGBtoHSB(col.getRed(), col.getGreen(), col.getBlue(), hsv);

            Color col2 = Color.getHSBColor(hsv[0], 1, 1);
            opG.setColor(col2);
            opG.fillOval(x - th, y - th, th * 2, th * 2);
        }
    }

    private void drawPlots(TreeMap<Float, Color> v2col, HashMap<Color, Integer> col2num, double maxNum, boolean shadow) {
        double rff = 0.01;
        double rf = ((double) w) * rff;
        double zff = 0.02;
        double zf = ((double) w) * zff;
        double minww = ((double) w) / 500.0;
        float ww = (float) w;
        float hh = (float) h;
        double allCol360 = 180.0/360.0;
        double height = ((ww*hh)/(float)col2num.size())/allCol360;
//        for (Float v : v2col.keySet()) {
//            Color col = v2col.get(v);
        double x = 0;
        double y = 0;
        List<Color> allCols = new ArrayList();
        for (Color col : col2num.keySet()) {
            allCols.add(col);
        }

        float t= 3600;
        int ind = 0;
        Collections.sort(allCols, new Comparator<Color>() {
            @Override
            public int compare(Color o1, Color o2) {
                int r1 = o1.getRed();
                int g1 = o1.getGreen();
                int b1 = o1.getBlue();
                int r2 = o2.getRed();
                int g2 = o2.getGreen();
                int b2 = o2.getBlue();

                float[] hsv1 = new float[3];
                Color.RGBtoHSB(r1, g1, b1, hsv1);
                float[] hsv2 = new float[3];
                Color.RGBtoHSB(r2, g2, b2, hsv2);

                return (int)(t * hsv2[ind]) - (int)(t * hsv1[ind]);
            }
        });

        double size = (double)allCols.size();
        double shift = -1000.0;
        Collections.rotate(allCols, (int)shift);
        allCols = allCols.subList(0, (int)(allCol360*(double)allCols.size()));

        size = (double)allCols.size();
        double shiftf = shift/size;

        for (Color col : allCols) {
            float hf = (float)((1+shiftf) + ((y*ww) + x)/(ww*hh));
            Color ccc = Color.getHSBColor(hf, 1f, 1f);
            float n = col2num.get(col)/((float) maxNum);
            n = (float)Math.pow(n,0.5);
            float r = ccc.getRed()/255f;
            float g = ccc.getGreen()/255f;
            float b = ccc.getBlue()/255f;

            Color cc = new Color(r, g, b, 1-n);
            opG.setColor(cc);
            Rectangle2D.Double rect = new Rectangle2D.Double(x, y, 1, height);
            opG.fill(rect);
            x++;
            if (x>w) {
                y = y + (int)(height);
                x = 0;
            }
        }
    }

    private void initFile() throws IOException {
        ibi = ImageIO.read(new File(ipF));


        obi = new BufferedImage(w, h, BufferedImage.TYPE_INT_RGB);
        opG = (Graphics2D) obi.getGraphics();
        opG.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        opG.setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BICUBIC);
        opG.setColor(Color.BLACK);
        opG.fillRect(0, 0, w, h);

    }


    private double[] getRefPositions(int th, int r, int g, int b) {

        double radius = (double) (-th*2 + w / 2);
        double cx = w / 2;
        double cy = h / 2;

        float[] hsv = new float[3];
        Color.RGBtoHSB(r, g, b, hsv);
        double h = hsv[0];
        double s = hsv[1];
        double v = hsv[2];
        double radians = h * Math.PI * 2 - Math.PI * 0.5;

        double x = radius * Math.cos(radians);
        double y = radius * Math.sin(radians);

        double[] arr = {cx + x, cy + y};
        return arr;

    }

    private double[] getPositions(int r, int g, int b) {
        float[] hsv = new float[3];

        double radius = 0.9 * (double) (w / 2);
        double cx = w / 2;
        double cy = h / 2;

        Color.RGBtoHSB(r, g, b, hsv);
        double h = hsv[0];
        double s = hsv[1];
        double v = hsv[2];
        double radians = h * Math.PI * 2 - Math.PI * 0.5;
        double sat = v * s * radius;
        double val = v;

        double x = sat * Math.cos(radians);
        double y = sat * Math.sin(radians);
        double z = val;
        double rr = Math.sqrt(x * x + y * y);
        x = x + cx;
        y = y + cy;

        double[] arr = {x, y, z, rr};
        return arr;
    }


}
